...
void solve(int tc)
{
    ...
    map<int,pair<int,int>> my;
    for(int i=0;i<min(n,4003ll);i++){
        for(int j=i+1;j<min(n,4003ll);j++){
            if(my.find(v[i]+v[j])==my.end()) my[v[i]+v[j]]={i,j};
            else{
                pair<int,int> p = my[v[i]+v[j]];
                if(p.ff!=i && p.ss!=j && p.ff!=j && p.ss!=i){
                    cout<<"YES"<<endl;
