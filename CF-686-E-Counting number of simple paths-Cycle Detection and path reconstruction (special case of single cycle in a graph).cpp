...
void solve()
{
	...
	vector<int> cycle;
	int start=-1, end=-1;
	std::function<void(int)> dfsCycle = [&](int node){
		vis[node] = 1;//mark
	...
	cycle.pb(start);
	set<int> check;
	for(auto ele:cycle) check.insert(ele);
	std::function<int(int,int)> dfs = [&](int node, int p){
		int cnt = 1;
		for(auto child:edges[node]){
			if(child==p || check.count(child)) continue;
			cnt+=dfs(child,node);
		}
		return cnt;
	};
	int res = 0;
	for(auto ele:cycle){
		int cnt_ele = dfs(ele,-1);
		// error(ele,cnt_ele);
		int a1 = (cnt_ele*(cnt_ele-1)/2);
		int a2 = (cnt_ele*(n-cnt_ele));
		res+=(a1+a2);
	}
	cout<<res<<endl;
}	
