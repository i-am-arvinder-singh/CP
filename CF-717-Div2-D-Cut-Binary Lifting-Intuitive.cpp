...
void solve(int tc)
{
    ...
    vector<int> prime_facts[MAX], next_fact(MAX,inf);
    vector<int> v(n);
    for(int i=0;i<n;i++) cin>>v[i];
    for(int i=2;i<MAX;i++) {
        if(prime_facts[i].empty()){
            prime_facts[i].pb(i);
    ...
    vector<vector<int>> dp(n+3,vector<int>(20,n));
    dp[n][0] = n; // Self loop
    for(int i=n-1;i>=0;i--){
        dp[i][0] = dp[i+1][0];
        for(auto ele:prime_facts[v[i]]){
            dp[i][0] = min(dp[i][0],next_fact[ele]);
            next_fact[ele] = i;
        }
//        cout<<"******* "<<i<<" "<<dp[i][0]<<" "<<endl;
    }
